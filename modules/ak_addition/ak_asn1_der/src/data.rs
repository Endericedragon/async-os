...
impl<'a, S: Source> Source for CountingSource<'a, S> {
    ...
    fn read(&mut self) -> Result<u8, Asn1DerError> {
        let e = self
            .source
            .read()
            .propagate(e!("Failed to read element from underlying source"))?;
        match self.ctr.checked_add(1) {
            ...
            _ => Err(eio!(
                "Cannot read more because the position counter would overflow"
            )),
        }
    }
}
...
impl<S: Source, U: Sink> Source for CopyingSource<S, U> {
    ...
    fn read(&mut self) -> Result<u8, Asn1DerError> {
        ...
        self.sink
            .write(e)
            .propagate(e!("Failed to copy element to sink"))?;
        Ok(e)
    }
}
...
#[cfg(all(not(feature = "std"), not(feature = "no_panic")))]
impl Sink for Vec<u8> {
    fn write(&mut self, e: u8) -> Result<(), Asn1DerError> {
        self.push(e);
        Ok(())
...
impl<'a, S: Sink> Sink for CountingSink<'a, S> {
    ...
    fn write(&mut self, e: u8) -> Result<(), Asn1DerError> {
        self.sink
            .write(e)
            .propagate(e!("Failed to write element to underlying source"))?;
        *self.ctr = match self.ctr.checked_add(1) {
            Some(ctr_next) => ctr_next,
            None => Err(eio!(
                "Cannot write more because the position counter would overflow"
            ))?,
        };
        Ok(())
    }
